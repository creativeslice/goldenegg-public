...
<section id="<?php echo $block_id; ?>" class="<?php echo $block_classes; ?>">
	...
	<?php if (get_field('cards')) : ?>
		...
		<?php while ( have_rows( 'cards' ) ) : the_row();
			...
			$card_classes = 'card-acf ' . $image_alignment;
			if( $card_content == '') {
				$card_classes .= ' noText';
			} elseif( $card_image == '') {
		...
		?>
			...
			if($card_link) : ?>
				<a href="<?php echo $card_link['url']; ?>" target="<?php echo $card_link['target']; ?>" class="<?php echo $card_classes; ?>" data-aos="fade-up" data-aos-duration="1200">
					...
					<?php if($card_icon) { ?>
						<svg class="card-icon"><use href="<?php echo get_svg($card_icon); ?>"></use></svg>
					<?php } ?>
					...
					<?php if($card_content) { ?>
						<div class="card-content">
							<p class="has-normal-font-size"><?php echo $card_content; ?></p>
						</div>
					<?php } ?>
