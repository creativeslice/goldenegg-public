...
let environment = 'prod'; // always default to production
let compress = 'compressed'; // default to compressed
...
gulp.task( 'scripts', function () {
	const customScripts = filter(
		'src/js/**/*.js' + 'partials/**/*.js' + 'blocks/**/*.js',
		{ restore: true }
	);
	return gulp
		.src( [
...
gulp.task( 'scripts-admin', function () {
	return gulp
		.src( [ 'src/libs/*.js', 'blocks/**/*.js', 'src/js/admin-only.js' ] )
		.pipe( plumber( { errorHandler: onError } ) )
			...
			.pipe(
				cheerio( {
					run( $, file ) {
						$( 'svg' ).addClass( 'hide' );
						$( 'symbol[id!=logo]' )
							.find( 'path,g,polygon,circle,rect' )
							.removeAttr( 'fill' );
					},
